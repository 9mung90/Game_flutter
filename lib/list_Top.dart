...
class ListTop extends StatefulWidget {                    // 🔁 이름 변경
  final Game game;
  const ListTop({super.key, required this.game});        // 🔁 생성자 이름 변경

  @override
  State<ListTop> createState() => _ListTopState();        // 🔁 State 타입/이름 변경
}

class _ListTopState extends State<ListTop> {              // 🔁 State 클래스 이름 변경
  final TextEditingController _searchController = TextEditingController();
  final PageController _pageController = PageController(initialPage: 0);      // ⭐ 페이지 스와이프 컨트롤러
  final ScrollController _tabScrollController = ScrollController();           // ⭐ 카테고리 탭 스크롤 컨트롤러
  ...
  void dispose() {
    ...
    _tabScrollController.dispose(); // ⭐ 추가
    super.dispose();
  }
  ...
  void _scrollToCategory(int index) {
    ...
    // _buildCategoryButton 기준:
    //   width: 50
    //   margin.horizontal: 4 + 4 = 8
    // → 버튼 하나당 대략 58px 정도 차지한다고 가정
    const double itemWidth = 58.0;
    final double screenWidth = MediaQuery.of(context).size.width;

    // 선택된 버튼이 화면 중앙 근처에 오도록 타겟 오프셋 계산
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> _pages = [
      EWeaponListPage(
        ...
        genreFilter: _weaponGenreFilter, // 🔹 무기 필터 값 전달
      ),
      EArmorListPage(
        game: widget.game,
        searchQuery: _searchQuery,
        showImageDialog: _showImageDialog,
        partFilter: _armorPartFilter, // 🔹 방어구 필터 값 전달
      ),
      EAshListPage(
        game: widget.game,
        searchQuery: _searchQuery,
        showImageDialog: _showImageDialog,
        propertyFilter: _ashPropertyFilter, // 🔹 전투 기술(재) 필터 값 전달
      ),
      ESpellListPage(
        game: widget.game,
        searchQuery: _searchQuery,
      ...
      EEtcListPage(
        ...
        searchQuery: _searchQuery,
        showImageDialog: _showImageDialog,
        typeFilter: _etcTypeFilter, // 🔹 기타 필터 값 전달
      ),
    ];
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            // 🔹 개인 사진 (네모 모양)
            Image.asset(
              'assets/images/grace_Icon2.png', // 네가 넣은 사진 경로
              width: 32,
              height: 32,
              fit: BoxFit.cover,
            ),
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            child: ListView(
              controller: _tabScrollController,           // ⭐ 컨트롤러 연결
              ...
              children: <Widget>[
                _buildCategoryButton(
                  ...
                  onTap: (index) {
                    ...
                    _scrollToCategory(index);              // ⭐ 탭도 스크롤
                  },
                ),
                _buildCategoryButton(
          ...
          Expanded(
            ...
            child: PageView(
              ...
              onPageChanged: (index) {
                ...
                _scrollToCategory(index);                 // ⭐ 스와이프로 바뀔 때도 탭 자동 스크롤
              },
              children: _pages,
            ),
          ),
